...
#include "Edu_Hadronic_Current_R.h"
#include <algorithm>
#include <iostream>
...
Delta_in_medium::Delta_in_medium() {
    ...
    this->rho_avg = 3.0/(4.0*Edu_Param::pi*(1.2*1.2*1.2)) * this->hc3; // average density in fm^-3 * hc^3
...
double Delta_in_medium::Gamma_vacuum(double p2_in) {
    if(p2_in > (this->mn + this->mpi)*(this->mn + this->mpi)) {
        ...
        return 1.0/(6.0*Edu_Param::pi) * (this->coupling_DeltaNpi/this->mpi)*(this->coupling_DeltaNpi/this->mpi)
...
double Delta_in_medium::kf(double rho_r_in) {
    return pow(3.0 * Edu_Param::pi*Edu_Param::pi * rho_r_in/2.0, 1.0/3.0);
}
